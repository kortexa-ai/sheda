// Fixed version of sky.toy.glsl with proper coordinate handling
...
void main() {
    // Get normalized coordinates matching the working shader style
    vec2 uv = vUv;
    vec2 r = resolution;
    float t = time;
    ...
    // Special handling for d to match toy format
    // Original uses: vec3 d = vec3(FC.xy / r - .6, 1);
    // We'll use: d = vec3((uv - 0.5) * 1.2, 1.0)
    vec3 d = vec3((uv - 0.5) * 1.2, 1.0);
